...
func TestCopyRehashNilTrees(t *testing.T) {
	tree := NewMutableTree(dbm.NewMemDB(), 0, false, NewNopLogger())
	require.Error(t, CopyRehash(nil, tree))
	require.Error(t, CopyRehash(tree, nil))
}

func TestSHA256HasherMatchesLegacyEmptyHash(t *testing.T) {
	legacy := NewMutableTree(dbm.NewMemDB(), 0, false, NewNopLogger())
	explicit := NewMutableTree(dbm.NewMemDB(), 0, false, NewNopLogger(), HasherOption(hash.NewSHA256Hasher()))
	require.Equal(t, legacy.Hash(), explicit.Hash())
}
