...
func (nk *NodeKey) GetKey() []byte {
	b := make([]byte, 12)
	binary.BigEndian.PutUint64(b, uint64(nk.version)) // nolint:gosec // the integer version is always positive
...
func GetNodeKey(key []byte) *NodeKey {
	return &NodeKey{
		version: int64(binary.BigEndian.Uint64(key)), // nolint:gosec // the integer version is always positive
...
func GetRootKey(version int64) []byte {
	b := make([]byte, 12)
	binary.BigEndian.PutUint64(b, uint64(version)) // nolint:gosec // the integer version is always positive
...
func makeNode(nk, buf []byte, useBlake3 bool) (*Node, error) {
	...
	buf = buf[n:]
	height8 := int8(height) // nolint:gosec // we perform the check in the line below
...
func MakeLegacyNode(hash, buf []byte) (*Node, error) {
	...
	node := &Node{
		subtreeHeight: int8(height), //nolint: gosec // used in testing
		size:          size,
